...
Status AddRelation( LGraph *g, int v1_index, int v2_index, int len ) {
    if ( g == NULL )
        return INVALID_ARGUMENT;
    ArcNode *p = NULL;
    ArcNode *new = NULL;
    ...
    for ( int i = 0; i < MAX_VERTEX_NUM; i++ ) {
        ...
        while ( p ) {
            if( g->kind == UDG || g->kind == UDN )
                ...
                OUTPUT_VERTEX_WEIGHTED( p->vexindex, p->len );
            p = p->next;
        }
        printf( " |\n" );
    }
